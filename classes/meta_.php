<?php
namespace bb_multisite_content;
class metaClass {
    ...
    function bb_new_field($args) {
        ...
        if (!$title && !$field_name)
            return;
        if (!$title && $field_name) {
            $title = ucwords(strtolower(str_replace(array('_', '-'), ' ', $field_name)));
        }
        if (!$field_name && $title)
            $field_name = $title;
        ...
        switch ($type) {
            case 'checkbox':
                ...
                if ($source == 'meta') {
                    $current_value = get_post_meta($_GET['post'], $field_name, true);
                    $checked = empty($current_value) || $current_value == $value ? 'checked="checked"' : '';
                } elseif ($source == 'option') {
                    $option = get_option($group);
                    $current_value = $option[$name];
                    $checked = ($current_value == $value) ? 'checked="checked"' : '';
                }
                echo '   <input type="checkbox" name="' . $field_name . '" value="'.$value.'" ' . $checked . ' style="margin: 0 5px 0px 0;"/><label style="color:rgba(0,0,0,0.75);">' . $title . '</label>' . "\n";
                break;
